...
function Game() {
	...
	function Spawn() {
        ...
        var x = Math.floor( ( Math.random() * ( game.offsetWidth - 48 ) ) );
        var y = Math.floor( ( Math.random() * ( game.offsetHeight -  48 ) ) );
        ...
        setTimeout(function(){ Remove(fragment.id); }, ttl);
	}
	...
	this.Start = function() {
        ...
        function timer() {
			...
			if(count <= 0) {
                ...
                time.innerHTML = count + " seconds";
            }
        }

        counter = setInterval(timer, 1000); 

        setTimeout(function(){ Stop(interval); }, count * 1000);
    };
